...
public interface ApplicationRepository extends JpaRepository<Application, Long> {
    ...
    List<Application> findByEnvironment(Application.Environment environment);

    List<Application> findByApplicationNameContainingIgnoreCase(String applicationName);

    @Query("SELECT a FROM Application a WHERE a.client.id = :clientId AND " +
           "LOWER(a.applicationName) LIKE LOWER(CONCAT('%', :search, '%'))")
